...
function CSS3MultiColumn() {
  ...
  ut.XBrowserAddEventHandler(window,'load',function() { 
    ...
    debug("Paging Time: " + (new Date().getTime() - start) + "ms")
	}
  
  PagingArticle = processElements;
  PagingRules = pseudoCSSRules;
  Paging = function(cc, cw){
    if (!documentReady) return;
    pseudoCSSRules[".Article"]['column-count'] = cc;
    pseudoCSSRules[".Article"]['column-width'] = cw+'px';
    document.getElementById('Content').style.width = cc*(cw+20) - 20 +'px'
    processElements(); 
